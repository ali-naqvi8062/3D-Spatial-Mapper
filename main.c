...
- Utilizes a Time-of-Flight sensor to gather distance data via I2C communication protocl between
MCU and sensor.

- Distance data is transmitted through UART to the PC where python libraries such as pyserial, numpy
and open3D are used to organize and plot xyz coordinate data for visualization. 

- This implementation uses a bus clock speed of 40MHz.
...
uint8_t currentState = 0; // zero is the stepper motor IDLE state, 1 is the stepper motor SPINNING state
int lastState [1] = {1}; // for button debounce, up to n buttons.

uint16_t dev = 0x29; // address of the ToF sensor as an I2C slave peripheral
int status=0; // keeps track of returned values from ToF API functions

void getDistanceMeasurement(); // distance data gathering function prototype
...
//I2C spyline debugging port (input: PM0)
...
// onboard push button (PJ1) to be used to initiate data acqusition process
...
// LED PN0 and PN1 to be used as data transmission and spin function debugging units.
...
//stepper motor port inputs PH0:3
void PortH_Init(void){
    ...
    GPIO_PORTH_DEN_R |= 0x0F; // enable digital I/O on Port H pins (PH0-PH3)                                                                                                                                    // configure Port H as GPIO
...
int checkButtonPress(uint32_t port, uint32_t pin){ // is not just limited to 1 button, can be used for any amount
    ...
    int curr = port & mask; // Read the current state of button
    ...
    if((curr != lastState[pin]) && (curr == 0)) // If button state changed (happens when curr=0 due to active LO push btn)
    {
        ...
        curr = port & mask; // Re-read the button state after delay
...
void spin()
{
      ...
      for(int i=1; i <= 512; i++) // 512 * 4 = 2048, overall 1 full 360 degree rotation.
      {
            ...
            SysTick_Wait10ms(10);
            getDistanceMeasurement(); // gather data
            }
...
int main(void) {
    ...
    switch(currentState)
        ...
        case 1: //running
            ...
            unspin(); // return to home position
            break;
